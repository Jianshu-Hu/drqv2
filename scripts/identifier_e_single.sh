...
# prepare tmp environment
tmp_dir="/tmp/drqv2-xcc-$identifier-task$task-aug$aug_type-feat_aug$feat_aug_type-seed$seed"
if [ -d "$tmp_dir" ]; then
    rm -rf $tmp_dir
    echo "Deleted original tmp_dir: $tmp_dir"
fi
mkdir $tmp_dir
...
mkdir $ori_dir/$aug_dir/seed$seed
cp $tmp_dir/$result_folder/eval.csv $ori_dir/$aug_dir/seed$seed
